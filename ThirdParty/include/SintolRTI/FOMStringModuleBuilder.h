...
#ifndef SintolRTI_FOMStringModuleBuilder_h
#define SintolRTI_FOMStringModuleBuilder_h
...
class SintolRTI_LOCAL FOMStringModuleBuilder {
public:
  ...
  void validate()
  {
    ...
    SintolRTIAssert(_parentInteractionClassIndexVector.size() == _module.getInteractionClassList().size());
    for (size_t i = 0; i < _parentInteractionClassIndexVector.size(); ++i) {
      ...
      if (parentIndex == ~std::size_t(0))
        continue;
      SintolRTIAssert(parentIndex < _parentInteractionClassIndexVector.size());
      StringVector stringVector = _module.getInteractionClassList()[parentIndex].getName();
      SintolRTIAssert(_module.getInteractionClassList()[i].getName().size() == 1);
    ...
    SintolRTIAssert(_parentObjectClassIndexVector.size() == _module.getObjectClassList().size());
    for (size_t i = 0; i < _parentObjectClassIndexVector.size(); ++i) {
      size_t parentIndex = _parentObjectClassIndexVector[i];
      if (parentIndex == ~std::size_t(0))
        continue;
      SintolRTIAssert(parentIndex < _parentObjectClassIndexVector.size());
      StringVector stringVector = _module.getObjectClassList()[parentIndex].getName();
      SintolRTIAssert(_module.getObjectClassList()[i].getName().size() == 1);
