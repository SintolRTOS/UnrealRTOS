...
#ifndef SintolRTI_Ambassador_h
#define SintolRTI_Ambassador_h
...
template<typename T>
class SintolRTI_LOCAL Ambassador : public InternalAmbassador {
public:
  typedef T Traits;
    ...
    //        RTIinternalError)
  {
    ...
    SintolRTIAssert(objectClass->getEffectiveSubscriptionType() == Unsubscribed);
    ...
    //        RTIinternalError)
  {
    ...
    for (std::set<std::string>::const_iterator i = objectInstanceNameSet.begin(); i != objectInstanceNameSet.end(); ++i) {
      ...
      SintolRTIAssert(objectInstanceHandle.valid());
      message->getObjectInstanceHandleVector().push_back(objectInstanceHandle);
    }
    send(message);
  }
    ...
    //        RTIinternalError)
  {
    ...
    for (std::vector<SintolRTI::AttributeValue>::iterator i = attributeValues.begin(); i != attributeValues.end(); ++i) {
  ...
  class SintolRTI_LOCAL _CallbackDispatchFunctor {
  public:
  ...
  void acceptCallbackMessage(const DeleteObjectInstanceMessage& message)
  {
    ...
    if (objectClass) {
      if (Unsubscribed != objectClass->getEffectiveSubscriptionType()) {
        removeObjectInstance(message.getObjectInstanceHandle(), message.getTag(), SintolRTI::RECEIVE, message.getFederateHandle());
      }
    }
  ...
  void acceptCallbackMessage(const AttributeUpdateMessage& message)
  {
    ...
    reflectAttributeValues(*objectClass, message.getObjectInstanceHandle(), message.getAttributeValues(), message.getTag(),
                           SintolRTI::RECEIVE, message.getTransportationType(), message.getFederateHandle());
  }
