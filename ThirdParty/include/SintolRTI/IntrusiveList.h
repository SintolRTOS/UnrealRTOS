/* -*-c++-*- SintolRTI - Copyright (C) 2013-2014 Mathias Froehlich
 *
...
#ifndef SintolRTI_IntrusiveList_h
#define SintolRTI_IntrusiveList_h
...
class SintolRTI_LOCAL _IntrusiveListHook {
public:
  _IntrusiveListHook()
  { _clear(); }
  // Don't copy list membership
  _IntrusiveListHook(const _IntrusiveListHook& intrusiveListHook)
  { _clear(); SintolRTIAssert(!intrusiveListHook.is_linked()); }
  ~_IntrusiveListHook()
  { _unlink(); }

  // Don't copy list membership
  _IntrusiveListHook& operator=(const _IntrusiveListHook& intrusiveListHook)
  { SintolRTIAssert(!intrusiveListHook.is_linked()); return *this; }

  bool is_linked(void) const
  {
    SintolRTIAssert((_next == this) == (_prev == this));
    return _next != this;
  }
...
private:
  ...
  { SintolRTIAssert(!is_linked()); return _insert_unchecked(intrusiveListHook); }
...
template<typename T, typename Tag>
class SintolRTI_LOCAL _IntrusiveListIterator :
...
template<typename T, typename Tag>
class SintolRTI_LOCAL _ForwardIntrusiveListIterator : public _IntrusiveListIterator<T, Tag> {
...
template<typename T, typename Tag>
class SintolRTI_LOCAL _ReverseIntrusiveListIterator : public _IntrusiveListIterator<T, Tag> {
  typedef _IntrusiveListIterator<T, Tag> _Base;
public:
  typedef typename _Base::value_type value_type;
  typedef typename _Base::difference_type difference_type;
...
template<typename T, typename Tag>
class SintolRTI_LOCAL _IntrusiveList {
public:
  ...
  { SintolRTIAssert(!empty()); SintolRTIAssert(_select(*i).is_linked()); delete i++.get(); return i; }
  ...
  void pop_front()
  { SintolRTIAssert(!empty()); erase(begin()); }
  void pop_back()
  { SintolRTIAssert(!empty()); erase(rbegin()); }

  /// Unlinks the front/back entry of this list from the list. Does not delete the entry.
  iterator unlink(iterator i)
  { SintolRTIAssert(!empty()); SintolRTIAssert(_select(*i).is_linked()); _select(*i++)._unlink_clear(); return i; }
  ...
  /// Unlinks the front/back entry of this list from the list. Does not delete the entry.
  void unlink_front()
  { SintolRTIAssert(!empty()); unlink(begin()); }
  void unlink_back()
  { SintolRTIAssert(!empty()); unlink(rbegin()); }

  /// Access to the first entry in the list
  const_reference front() const
  { SintolRTIAssert(!empty()); return *begin(); }
  reference front()
  { SintolRTIAssert(!empty()); return *begin(); }

  /// Access to the last entry in the list
  const_reference back() const
  { SintolRTIAssert(!empty()); return *rbegin(); }
  reference back()
  { SintolRTIAssert(!empty()); return *rbegin(); }
  ...
  { SintolRTIAssert(_select(t).is_linked()); return iterator(&_select(t)); }
  static const_iterator it(const_reference t)
  { SintolRTIAssert(_select(t).is_linked()); return const_iterator(const_cast<Hook*>(&_select(t))); }
...
class SintolRTI_LOCAL _ListTag {};

template<typename T, unsigned tag = 0>
class SintolRTI_LOCAL IntrusiveList : public _IntrusiveList<T, _ListTag<T, tag> > {
  typedef _IntrusiveList<T, _ListTag<T, tag> > _Implementation;
public:
  IntrusiveList()
  { }
  IntrusiveList(const IntrusiveList& intrusiveList)
  { SintolRTIAssert(intrusiveList.empty()); }
...
#endif
  ~IntrusiveList()
  { SintolRTIAssert(_Implementation::empty()); }

  IntrusiveList& operator=(const IntrusiveList& intrusiveList)
  { SintolRTIAssert(_Implementation::empty()); SintolRTIAssert(intrusiveList.empty()); return *this; }
#if 201103L <= __cplusplus || 200610L <= __cpp_rvalue_reference
