...
#ifndef SintolRTI_IntrusiveUnroderedMap_h
#define SintolRTI_IntrusiveUnroderedMap_h
...
template<typename T>
struct SintolRTI_LOCAL Hash {
  std::size_t operator()(const T& value) const
  { return 0; }
};

template<>
struct SintolRTI_LOCAL Hash<std::string> {
...
template<>
struct SintolRTI_LOCAL Hash<std::vector<std::string> > {
...
class SintolRTI_LOCAL IntrusiveUnorderedMap {
  class SintolRTI_LOCAL _TotalListTag {};
  class SintolRTI_LOCAL _BucketListTag {};
...
public:
  ...
  class SintolRTI_LOCAL Hook : public _List::Hook, public _BucketList::Hook {
  public:
    Hook() :
      _key()
    { }
    ...
    { SintolRTIAssert(_List::Hook::is_linked() == _BucketList::Hook::is_linked()); return _List::Hook::is_linked(); }
  ...
  protected:
    ...
    { SintolRTIAssert(!is_linked()); _key = key; }
  ...
  { SintolRTIAssert(intrusiveUnorderedMap.empty()); }
  ...
  { SintolRTIAssert(empty()); SintolRTIAssert(intrusiveUnorderedMap.empty()); return *this; }
#if 201103L <= __cplusplus || 200610L <= __cpp_rvalue_reference
