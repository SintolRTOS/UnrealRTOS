...
#ifndef SintolRTI_VariableLengthData_h
#define SintolRTI_VariableLengthData_h
...
class SintolRTI_API VariableLengthData {
public:
  VariableLengthData() :
    _data(0),
    _size(0),
  ...
  { SintolRTIAssert(offset + size <= value.size()); }
  ...
  {
    SintolRTIAssert(_data.valid() || _size == 0);
    if (!_data.valid())
      return 0;
    SintolRTIAssert(offset < _size);
  ...
  {
    SintolRTIAssert(offset + variableLengthData.size() <= size());
  ...
  {
    SintolRTIAssert((offset & 1) == 0);
#if defined SintolRTI_HOST_IS_BIG_ENDIAN
    setUInt16LE(value, offset);
#elif defined SintolRTI_HOST_IS_LITTLE_ENDIAN
    SintolRTIAssert(offset + 2 <= size());
...
private:
  class SintolRTI_API Data : public Referenced {
  public:
    Data(size_t size) :
      _capacity(size),
      _data(_dummy)
